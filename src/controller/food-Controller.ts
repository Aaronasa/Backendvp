...
export class FoodController {
  static async createFood(req: Request, res: Response): Promise<void> {
    try {
      ...
      const request = FoodValidation.CREATE.parse(req.body);
      ...
      console.error(error);
      res.status(400).json({ error: error instanceof Error ? error.message : "An error occurred while creating the food." });
    }
  }

  static async readFoodById(req: Request, res: Response): Promise<void> {
    try {
      const params = FoodValidation.READ_BY_ID.parse({ id: parseInt(req.params.id, 10) });
      ...
      console.error(error);
      res.status(400).json({ error: 'An error occurred while retrieving the food.' });
    }
  }
  ...
  static async readAllFoods(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(error);
      res.status(500).json({ error: 'An error occurred while retrieving foods.' });
    }
  }

  static async updateFood(req: Request, res: Response): Promise<void> {
    try {
      const request = FoodValidation.UPDATE.parse(req.body);
      ...
      console.error(error);
      res.status(400).json({ error: 'An error occurred while updating the food.' });
    }
  }
  ...
  static async deleteFood(req: Request, res: Response): Promise<void> {
    try {
      ...
      console.error(error);
      res.status(400).json({ error: 'An error occurred while deleting the food.' });
    }
  }
}
